            ...
            if num >0:
                for n in range(num):# смотрим письма и сравниваем тему письма с заданной
                    msglines = pop.retr(n+1)[1]
                                                #текст письма во втором элементе кортежа
                    msgtext = "\n".join(map(lambda x: x.decode("utf-8"), msglines))
                    #строчки склеиваем вместе и получаем текст, но мы имеем дело с байтовыми строками,
                   ...
                   # print (a)
                    ...
                    # помечаем письмо на удаление
                        pop.quit()
                    # закрытие с сохранением, письма помеченные на удаление удалятся только при закрытии
